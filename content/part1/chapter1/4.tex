为了我们的第一个测试项目，我们将创建一个简单的Clang工具，该工具运行编译器并检查提供的源文件的语法。我们将创建一个所谓的树外LLVM项目，即一个将使用LLVM但位于主LLVM源树外的项目。
...
\begin{itemize}
\item
第2行：我们指定了项目名称（syntax-check）。这同时也是我们可执行文件的名称。

\item
第4-7行：测试LLVM\_HOME环境变量。

\item
第10行：我们设置了LLVM CMake帮助器的路径。

\item
第11行：我们从第10行指定的路径加载LLVM CMake包。

\item
第14行：我们指定了我们应该编译的源文件。

\item
第16行：我们设置了额外的编译标志：-fno-rtti。当LLVM没有构建RTTI时，这个标志是必需的。这是为了减少代码和可执行文件大小而进行的努力 \footnote{LLVM社区. LLVM编码标准. 2023. URL \url{https://llvm.org/docs/CodingStandards.html}。}。

\item
第18-22行：我们指定要链接到我们程序的必需库
\end{itemize}
...
\begin{center}
图 1.16: The syntax-check –help 输出
\end{center}

如果我们在一个有效的C++源文件上运行程序，它将依次退出；但如果在一个有语法错误的C++文件上运行，它会产生一个错误消息：
...
我们在图1.17中使用'-{}-'将附加参数传递给编译器，具体指定我们想要使用C++17，选项'-std=c++17'。

我们还可以使用LLDB调试器运行我们的工具：
...
\begin{center}
图 1.18: 在调试器下运行syntax-check
\end{center}

我们以syntax-check为主程序，将main.cpp源文件作为工具的参数（图1.18）。我们还向syntax-check可执行文件传递了附加的编译标志(-std=c++17)。
