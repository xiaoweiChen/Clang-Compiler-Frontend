...
此外，脚本还为我们的检查生成了一个LIT测试，名为ClassChecker.cpp。这个测试可以在clang-tools-extra/test/clang-tidy/checkers/misc目录中找到。
...
在用自定义源代码测试文件后，是时候为我们的检查创建一个LIT测试了。

\mySubsubsection{5.4.3}{LIT测试}

对于LIT测试，我们将使用来自图4.43的稍作修改的代码。让我们修改位于clang-tools-extra/test/clang-tidy/checkers/misc文件夹中的classchecker.cpp文件，如下所示：
...
我们可以看到，与图4.43的唯一区别在于第1行，我们在这里指定了应该运行的命令，以及第9行，我们在这里定义了测试模式。
...
编译错误的情况可以被视为一个边缘情况，我们可以正确处理它。在处理这些情况之前，我们应该调查产生的AST以检查问题。

\mySubsubsection{5.4.5}{编译错误作为边缘情况}

让我们使用clang-query（参见第3.6节，使用clang-query探索Clang AST）来探索发生了什么。修复错误的程序如下所示：
...
然后，我们将Clang-Query的输出设置为dump，并找到所有匹配的CXXRecordDecl
