我们将使用在第5.4节中获得的知识来创建一个自定义的CFG检查。如前所述，该检查将使用Clang的CFG来计算循环复杂度。检查将在计算出的复杂度超过阈值时发出警告。这个阈值将作为一个配置参数设置，以便在测试期间进行更改。让我们从创建项目框架开始。
...
根据代码，我们的检查将仅应用于函数声明，即clang::FunctionDecl。代码还可以扩展以支持其他C++构造。
